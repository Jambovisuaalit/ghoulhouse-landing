...
const CDP_PORT = 9445;
const USER_DATA_DIR = `/tmp/ghoulhouse-ga4-release-smoke-v2-${process.pid}`;
...
try {
  ...
  for (const width of widths) {
    ...
    const state = await evaluate(client, `(() => {
      ...
      const keyboardReachableWhileHidden = (el) => {
        if (!el || el.tabIndex < 0) return false;
        if (el.closest('details:not([open])')) return false;
        let node = el;
        while (node instanceof HTMLElement) {
          const s = getComputedStyle(node);
          if (node.hidden || node.inert || s.display === 'none' || s.visibility === 'hidden' || s.visibility === 'collapse') return false;
      ...
      const hiddenFocusable = [...document.querySelectorAll('.desktopNav a, .mobileNav a')].filter(keyboardReachableWhileHidden).length;
    ...
    assert(state.hiddenFocusable === 0, `${width}: hidden-but-keyboard-reachable nav item found`);
